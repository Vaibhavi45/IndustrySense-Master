...
import { Lock, User, Eye, EyeOff, LogIn, Gauge, CheckCircle2, ArrowLeft } from "lucide-react"
import api from "@/lib/api"

export default function LoginPage() {
  const router = useRouter()
  const [username, setUsername] = useState("admin")
  const [password, setPassword] = useState("HueHueHue#69")
      ...
      <div className="w-full max-w-md mx-auto animate-fade-in-up relative z-10">
        <div className="backdrop-blur-xl bg-white/80 border border-white/20 rounded-2xl p-8 shadow-2xl">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold gradient-text mb-2">IndustriSense</h1>
            <p className="text-gray-600 text-sm">Smart Maintenance Management</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              ...
              <span>{loading ? "Signing In..." : showSuccess ? "Success!" : "Sign In"}</span>
            </button>
          </form>
          ...
          <div className="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl">
            <p className="font-semibold text-xs text-gray-700 mb-2">Demo Credentials</p>
            <div className="space-y-1 text-xs text-gray-600">
              <p>Username: <code className="bg-white/50 px-2 py-1 rounded text-blue-700 font-mono">admin</code></p>
              <p>Password: <code className="bg-white/50 px-2 py-1 rounded text-blue-700 font-mono">HueHueHue#69</code></p>
            </div>
          </div>
        </div>
      </div>
